...
class Server2Screen extends StatefulWidget {
  const Server2Screen({super.key});

  @override
  State<Server2Screen> createState() => _Server2ScreenState();
}

class _Server2ScreenState extends State<Server2Screen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // Load autopost configurations for Server 2 with token information
  ...
  void _handleRealtimeUpdate(List<dynamic> data) {
    ...
    try {
      ...
      final server2Configs = safeData.where((config) => config['server_type'] == 'server_2').toList();
    ...
    setState(() {
      ...
      // Update server 2 configs with token information
      _autopostConfigs = server2Configs.map((config) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(_isSelectionMode 
              ...
              : languageProvider.getLocalizedText('server_2')),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    languageProvider.getLocalizedText('basic_plan_access'),
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              'Create your first autopost configuration to get started with Server 2.',
              textAlign: TextAlign.center,
              style: Theme.of(context).textTheme.bodySmall?.copyWith(
                color: Theme.of(context).textTheme.bodySmall?.color?.withValues(alpha: 0.7),
              ),
  ...
  void _showCreateConfigDialog(LanguageProvider languageProvider) async {
    final result = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (context) => const AddConfigScreen(serverType: 'server_2'),
      ),
    );
